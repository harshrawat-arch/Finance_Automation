...
def process_revenue_report():
    try:
        ...
        # --------------------------------------------------------
        # 🔥 FIX APPLIED: Remove duplicate MID_WID from MM.csv
        # --------------------------------------------------------
        if os.path.exists(MM_FILE):
            ...
            payment_df = pd.merge(
                payment_df,
                mm_df[['MID_WID', 'SAP_New_Code', 'Nodal']],
                on='MID_WID',
                how='left'
            )
        ...
        if os.path.exists(RECOVERY_FILE):
            ...
            payment_df = pd.merge(
                payment_df, rec_grouped,
                left_on='m_id_str', right_on='Merchant_ID', how='left'
            )
            payment_df.rename(columns={'TOTAL': 'recovery'}, inplace=True)
        else:
            payment_df['recovery'] = 0

        # Payment logic (Updated with Recovered_pg and Recovered_cod)
        payment_df['recovery'] = pd.to_numeric(payment_df['recovery'], errors='coerce').fillna(0)

        # New Column: Recovered_pg
        payment_df['Recovered_pg'] = np.where(
            payment_df['pg_payable2'] <= 0, 
            0, 
            np.minimum(payment_df['pg_payable2'], payment_df['recovery'])
        ) * -1

        # New Column: Recovered_cod
        # FIX: Changed .clip(min=0) to .clip(lower=0) to resolve the error
        ...
        print("-------------------------------------------------")
        print("✅ SUCCESS — Recovery columns and duplicate MID_WID fixed")
        print("-------------------------------------------------")
