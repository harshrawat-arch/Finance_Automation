...
# ================= HOLD DETAILS EMAIL (NEW COLUMNS ADDED) ================= #
def prepare_hold_email(date_display):
    ...
    # Store original Nodal_Status before numeric conversion for the report
    if "Nodal_Status" in df.columns:
        df["Nodal_is_numeric"] = pd.to_numeric(df["Nodal_Status"], errors="coerce").notna()
    else:
        df["Nodal_is_numeric"] = True
    ...
    # Filter Logic
    hold_df = df[
        (df["Hold"].astype(str).str.lower() == "hold") | 
        (~df["Nodal_is_numeric"])
    ].copy()
    ...
    # Added Nodal_Status and Hold to the column list
    ...
    # Save the HOLD Excel Sheet with requested columns
    ...
    body = f"""
    <html>
    <style>.styled-table {{ border-collapse: collapse; font-size: 13px; width: 100%; }} .styled-table th {{ background-color: #791f1f; color: white; padding: 6px; border: 1px solid black; }} .styled-table td {{ padding: 6px; border: 1px solid black; text-align: right; }}</style>
    <body>
        <p>Dear Team,</p>
        <p>Please find the <b>HOLD Details</b> for the payout dated {date_display}. Record details including Nodal Status and Hold reason are attached.</p>
        {html_table}
    ...
    msg['Subject'] = f"HOLD Details for the payout dated {date_display}"
    ...
    if os.path.exists(HOLD_EXCEL_FILE):
        with open(HOLD_EXCEL_FILE, "rb") as f:
            ...
            part.add_header("Content-Disposition", f"attachment; filename={os.path.basename(HOLD_EXCEL_FILE)}")
            msg.attach(part)
...
def send_gmail():
    try:
        ...
        server.quit()
        print("✅ SUCCESS: Emails sent with updated HOLD columns.")
