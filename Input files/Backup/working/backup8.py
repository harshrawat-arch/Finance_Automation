...
def process_revenue_report():
    try:
        ...
        # 2. Load CSV Data with FIX for 'utf-8' error
        ...
        # Mapping SAP, Hold, and Recovery
        if os.path.exists(MM_FILE):
            mm_df = pd.read_csv(MM_FILE, encoding='latin1')
            mm_df['MID_WID'] = mm_df['MID_WID'].astype(str)
            payment_df = pd.merge(payment_df, mm_df[['MID_WID', 'SAP_New_Code']], on='MID_WID', how='left')
            payment_df.rename(columns={'SAP_New_Code': 'SAP'}, inplace=True)
        else:
            payment_df['SAP'] = ""
        ...
        if os.path.exists(HOLD_FILE):
            ...
            hold_df['Hold'] = "Hold"
            payment_df = pd.merge(payment_df, hold_df[['MID_WID', 'Hold']], on='MID_WID', how='left')
        else:
            payment_df['Hold'] = ""
        ...
        # 1. net pg_payable = pg_payable2 - recovery
        payment_df['net pg_payable'] = (payment_df['pg_payable2'] - payment_df['recovery']).round(2)
        
        # 2. net_cod_payable = cod_payable2 - balance recovery
        payment_df['balance_recovery'] = np.where(payment_df['recovery'] > payment_df['pg_payable2'], 
                                                  payment_df['recovery'] - payment_df['pg_payable2'], 0)
        payment_df['net_cod_payable'] = (payment_df['cod_payable2'] - payment_df['balance_recovery']).round(2)

        # Select exact headers for Payment Sheet in required order
        pay_headers = ['MID_WID', 'SAP', 'merchant_id_name2', 'GMV', 'merchant_payable2', 
                       'pg_payable2', 'cod_payable2', 'Hold', 'recovery', 
                       'net pg_payable', 'net_cod_payable']
        ...
        print("-" * 40)
        print(f"✅ SUCCESS! Files created in output_files folder.")
        print("-" * 40)
