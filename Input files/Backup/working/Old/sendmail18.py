...
# ================= OPEN ORDERS EMAIL (FIXED WITH TOTAL_PRICE) ================= #
def prepare_open_orders_email(date_display):
    ...
    # Calculate Total_Price: ((price * qty_ordered) + shipping_amount + pf_tax + pf_packing + pf_seller_convenience + cart_conv_fee)
    # Using .get() and fillna(0) to prevent errors if a column is missing
    price = pd.to_numeric(df.get('price', 0), errors='coerce').fillna(0)
    qty = pd.to_numeric(df.get('qty_ordered', 1), errors='coerce').fillna(1) # Defaulting to 1 if qty missing
    ...
    df['Total_Price'] = (price * qty) + shipping + tax + packing + seller_conv + cart_conv
    ...
    requested_cols = [
        ...
        'pf_seller_convenience', 'cart_conv_fee', 'vertical_id_name', 'product_id', 'Total_Price'
    ]
...
def send_gmail():
    try:
        ...
        server.quit()
        print("✅ SUCCESS: Emails sent. Total_Price column added to Open Orders.")
