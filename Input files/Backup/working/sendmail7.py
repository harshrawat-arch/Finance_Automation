...
def generate_payment_summary():
    ...
    # ⭐ HOLD(-) OTHER
    df_other = df[
        (df["HOLD"].astype(str).str.upper() == "HOLD") &
        (~df["MID_WID"].isin(euronet_mids))
    ]

    ho_escrow = df_other["pg_payable2"].sum()
    ho_cod = df_other["cod_payable2"].sum()

    # ⭐ ADDED-RECOVERY (negative values * -1)
    ad_escrow = df[df["pg_payable2"] < 0]["pg_payable2"].sum() * -1
    ad_cod = df[df["cod_payable2"] < 0]["cod_payable2"].sum() * -1

    # ⭐ PAYABLE AMOUNT = As Per Panel - hold_euronet - hold_other + added_recovery ✔ FIXED
...
def prepare_email(subject_line, date_display):
    ...
    <body>
        ...
        <h3>Payment Sheet Summary</h3>
        {payment_summary_html}
        <br>

        <h3>Payout Summary ({date_display})</h3>
        {html_table}
...
def send_gmail():
    try:
        ...
        server.quit()
        print("✅ SUCCESS: All emails sent with styled HTML tables.")
