...
def generate_payment_summary(is_main=False):
    ...
    # --- EXCLUSION LOGIC ---
    # Exclude these MIDs completely so they don't impact ANY totals
    excluded_mids = ["1530543", "870346", "1530540"]
    df = df[~df["MID_WID"].isin(excluded_mids)].copy()

    # Numeric conversion for payables
    ...
    # 4. Added-Recovery
    ad_escrow_neg = df[df["pg_payable2"] < 0]["pg_payable2"].sum() * -1
    ad_cod_neg = df[df["cod_payable2"] < 0]["cod_payable2"].sum() * -1
    
    # 5. adj amount Recov
    adj_recov_escrow = df["recovery"].sum() 
    adj_recov_cod = 0 

    # Final Payable amount Calculation
    payable_escrow = as_escrow - he_escrow - ho_escrow + ad_escrow_neg - adj_recov_escrow
    payable_cod = as_cod - he_cod - ho_cod + ad_cod_neg - adj_recov_cod
    total_val = payable_escrow + payable_cod

    summary_df = pd.DataFrame({
        "Mode": ["As Per Panel", "Hold_Euronet", "Hold_Other", "Added-Recovery", "adj amount Recov", "Payable amount"],
        "ESCROW": [as_escrow, he_escrow, ho_escrow, ad_escrow_neg, adj_recov_escrow, payable_escrow],
        "COD": [as_cod, he_cod, ho_cod, ad_cod_neg, adj_recov_cod, payable_cod]
    })
...
def prepare_hold_email(date_display):
    ...
    # Exclude unwanted MIDs here as well for consistency
    df = df[~df["MID_WID"].isin(["1530543", "870346", "1530540"])]
    ...
    body = f"<html><style>.styled-table {{ border-collapse: collapse; font-size: 13px; width: 100%; }} .styled-table th {{ background-color: #791f1f; color: white; padding: 6px; border: 1px solid black; }} .styled-table td {{ padding: 6px; border: 1px solid black; text-align: right; }}</style><body><p>Dear Team,</p><p>Please find the HOLD Details for {date_display}.</p>{html_table}<p><br>Regards,<br>Finance Automation System</p></body></html>"
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL); msg['Subject'] = f"HOLD Details - {date_display}"; msg.attach(MIMEText(body, 'html'))
...
def prepare_open_orders_email(date_display):
    ...
    # Exclude unwanted MIDs here too
    if 'merchant_id' in df.columns:
        df['merchant_id'] = df['merchant_id'].astype(str).str.strip()
        df = df[~df['merchant_id'].isin(["1530543", "870346", "1530540"])]

    price = pd.to_numeric(df.get('price', 0), errors='coerce').fillna(0)
    qty = pd.to_numeric(df.get('qty_ordered', 1), errors='coerce').fillna(1)
    shipping = pd.to_numeric(df.get('shipping_amount', 0), errors='coerce').fillna(0)
    tax = pd.to_numeric(df.get('pf_tax', 0), errors='coerce').fillna(0)
    packing = pd.to_numeric(df.get('pf_packing', 0), errors='coerce').fillna(0)
    seller_conv = pd.to_numeric(df.get('pf_seller_convenience', 0), errors='coerce').fillna(0)
    cart_conv = pd.to_numeric(df.get('cart_conv_fee', 0), errors='coerce').fillna(0)

    df_new = df.copy()
    df_new['Total_Price'] = (price * qty) + shipping + tax + packing + seller_conv + cart_conv

    requested_cols = ['merchant_id', 'order_created_at', 'merchant_id_name', 'order_id', 'order_item_id', 'Total_Price']
    available_cols = [col for col in requested_cols if col in df_new.columns]

    df_dir1 = df_new[df_new.get('direction') == 1][available_cols]
    df_dir2 = df_new[df_new.get('direction') == 2][available_cols]
    ...
    body = f"<html><body><p>Dear Team,</p><p>Please find attached Open Orders for {date_display}.</p><p>Regards,<br>Finance Automation System</p></body></html>"
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL); msg['Subject'] = f"Payout _Open orders - {date_display}"; msg.attach(MIMEText(body, 'html'))
...
def prepare_filtered_email(subject_line, date_display, mid_wid):
    ...
    if df_filtered.empty:
        html_table = f"<p>No data found for {mid_wid}</p>"
    else:
    ...
    body = f"<html><body><p>Dear Team,</p>{html_table}<p>Regards,<br>Finance Automation System</p></body></html>"
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL); msg['Subject'] = subject_line; msg.attach(MIMEText(body, 'html'))
    return msg
...
def prepare_attachment_email(date_display):
    ...
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL); msg['Subject'] = f"payout file checking - {date_display}"
...
def send_gmail():
    try:
        ...
        server.send_message(prepare_email(f"MKPL REGULAR PAYOUT - {date_display}", date_display, is_main=True))
        ...
        server.send_message(prepare_filtered_email(f"GOOGLE PLAY PAYOUT - {date_display}", date_display, 1139089))
        server.send_message(prepare_filtered_email(f"Apple BRAND PAYOUT - {date_display}", date_display, 1182161))
        server.send_message(prepare_attachment_email(date_display))

        server.quit()
        print("✅ SUCCESS: Emails sent. MIDs 1530543, 870346, 1530540 excluded.")
