...
def create_checklist_pivot(OUTPUT_FILE, CHECKLIST_FILE):
    try:
        ...
        total_row_top = pd.DataFrame([{
            'direction_name': 'Total',
            'merchant_payable': pivot_sheet2_top['merchant_payable'].sum(),
            'pg_payable2': pivot_sheet2_top['pg_payable2'].sum(),
            'cod_payable2': pivot_sheet2_top['cod_payable2'].sum(),
            'Difference': pivot_sheet2_top['Difference'].sum()
        }])
        pivot_sheet2_top = pd.concat([pivot_sheet2_top, total_row_top], ignore_index=True)
        ...
        total_row_bottom = pd.DataFrame([{
            'direction_name': 'Total',
            'GMV': pivot_sheet2_bottom['GMV'].sum(),
            'Commission': pivot_sheet2_bottom['Commission'].sum(),
            'Tax': pivot_sheet2_bottom['Tax'].sum(),
            'cust_shipping_reversal': pivot_sheet2_bottom['cust_shipping_reversal'].sum(),
            'partial_shipping_reversal': pivot_sheet2_bottom['partial_shipping_reversal'].sum(),
            'pf': pivot_sheet2_bottom['pf'].sum(),
            'product_gst': pivot_sheet2_bottom['product_gst'].sum(),
            'TCS': pivot_sheet2_bottom['TCS'].sum(),
            'TDS': pivot_sheet2_bottom['TDS'].sum(),
            'Seller Payable': pivot_sheet2_bottom['Seller Payable'].sum(),
            'merchant_payable': pivot_sheet2_bottom['merchant_payable'].sum(),
            'Diff': pivot_sheet2_bottom['Diff'].sum()
        }])
        pivot_sheet2_bottom = pd.concat([pivot_sheet2_bottom, total_row_bottom], ignore_index=True)
        ...
        comp_data = [
            ...
            ["Merchant payout report", m_pg, m_cod, m_pg + m_cod],
        ...
        pivot_sheet2_file = pivot_sheet2_file.round(2)
        ...
        # ✅ Sheet1 Formatting Added
        ws1 = wb["Sheet1"]
        for r in ws1.iter_rows(min_row=1, max_row=ws1.max_row, min_col=1, max_col=ws1.max_column):
            for cell in r:
        ...
        def format_table(start_row, df_table):
            header_row = start_row
            data_end = start_row + len(df_table)

            for r in ws.iter_rows(min_row=header_row, max_row=data_end, min_col=1, max_col=len(df_table.columns)):
                for cell in r:
                    cell.border = thin_border
        ...
        print("✅ Sheet1 formatting added successfully.")
