...
PERIOD_FILE_PATH = r'input_files\Period.xlsx'
...
def prepare_shipping_email(period_val):
    if not os.path.exists(SHIPPING_PAYOUT_FILE): return None
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL)
    msg['Subject'] = f"Testing | Shipping_Payout orders || {period_val}"
    body = f"<html><body><p>Dear Team,</p><p>Please find the <b>Shipping Payout Orders Report</b> attached.</p></body></html>"
...
def prepare_payout_orders_email(period_val):
    if not os.path.exists(PAYOUT_ORDERS_FILE): return None
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL)
    msg['Subject'] = f"Testing | Payout open orders ||  {period_val}"
    body = f"<html><body><p>Dear Team,</p><p>Please find the <b>Payout Open Orders</b> attached.</p></body></html>"
    msg.attach(MIMEText(body, 'html'))
...
def prepare_hold_email(period_val):
    ...
    body = f"<html><body><p>Dear Team,</p><p>Please find the <b>HOLD Details</b> attached.</p>{html_table}<p>Regards,<br>Finance Automation System</p></body></html>"
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL)
    msg['Subject'] = f"Testing |HOLD Details || {period_val}"
    msg.attach(MIMEText(body, 'html'))
...
def prepare_filtered_email(subject_line, mid_wid):
    if not os.path.exists(PAYMENT_FILE): return None
    df = pd.read_excel(PAYMENT_FILE, sheet_name='Sheet1')
    df.columns = df.columns.str.strip()
    ...
    body = f"<html><body><p>Dear Team,</p><p>Payout details for {mid_wid}:</p>{html_table}</body></html>"
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL); msg['Subject'] = subject_line; msg.attach(MIMEText(body, 'html'))
    return msg

def prepare_attachment_email(period_val):
    files = [CHECKLIST_FILE, PAYMENT_FILE] 
    msg = MIMEMultipart(); msg['From'] = SENDER_EMAIL; msg['To'] = ", ".join(RECEIVER_EMAIL)
    msg['Subject'] = f"Testing | payout file for checking || {period_val}"
...
def send_gmail():
    try:
        ...
        # Read Period from Excel
        period_val = "TBD"
        if os.path.exists(PERIOD_FILE_PATH):
        ...
        print(f"Sending 1/7: MKPL REGULAR PAYOUT - {period_val}...")
        server.send_message(prepare_email(f"Testing | MKPL REGULAR PAYOUT || {period_val}", date_display, is_main=True))

        # 2. GOOGLE PLAY PAYOUT
        print(f"Sending 2/7: GOOGLE PLAY PAYOUT - {period_val}...")
        server.send_message(prepare_filtered_email(f"Testing | GOOGLE PLAY PAYOUT || {period_val}", 1139089))

        # 3. Apple BRAND PAYOUT
        print(f"Sending 3/7: Apple BRAND PAYOUT - {period_val}...")
        server.send_message(prepare_filtered_email(f"Testing | Apple BRAND PAYOUT || {period_val}", 1182161))

        # 4. HOLD Details
        print(f"Sending 4/7: HOLD Details - {period_val}...")
        hold_msg = prepare_hold_email(period_val)
        if hold_msg: server.send_message(hold_msg)

        # 5. Shipping Payout orders
        if shipping_exists:
            print(f"Sending 5/7: Shipping Payout orders - {period_val}...")
            ship_msg = prepare_shipping_email(period_val)
            if ship_msg: server.send_message(ship_msg)

        # 6. Payout checking files
        print(f"Sending 6/7: Payout checking files - {period_val}...")
        server.send_message(prepare_attachment_email(period_val))

        # 7. Payout Open Orders
        print(f"Sending 7/7: Payout Open Orders - {period_val}...")
        orders_msg = prepare_payout_orders_email(period_val)
        if orders_msg: server.send_message(orders_msg)

        server.quit()
        print(f"✅ SUCCESS: Process completed for {period_val}.")
